...
class Brisque:
    ...
    def local_mean(self):
        if self._local_mean is None:
            self._local_mean = scipy.signal.convolve2d(
                self.image, self.kernel, "same")
    ...
    def mscn(self):
        if self._mscn is None:
            c = 1 / 255
            self._mscn = (self.image - self.local_mean) / \
                (self.local_deviation + c)
        return self._mscn
    ...
    def calculate_features(self, mscn_type: MscnType):
        agg = AsymmetricGeneralizedGaussian(
            self.get_coefficients(mscn_type)).fit()
...
def calculate_features(image: PIL.Image, kernel_size, sigma) -> numpy.ndarray:
    ...
    downscaled_brisque = Brisque(
        downscaled_image,
        kernel_size=kernel_size,
        sigma=sigma)
    features = numpy.concatenate(
        [brisque.features, downscaled_brisque.features])
